...
public class TransactionWithFullNameDeserializer implements Deserializer<TransactionWithFullName> {

    private static final Logger log = LoggerFactory.getLogger(TransactionWithFullNameDeserializer.class);
     ...
     */
    public TransactionWithFullName deserialize(String topic, byte[] bytes) {
...
//                        objectMapper.readTree(bytes), this.getClass(), "/json/transaction.json");
                return objectMapper.readValue(bytes, TransactionWithFullName.class);
            } catch (IOException e) {
