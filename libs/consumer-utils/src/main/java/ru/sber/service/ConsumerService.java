...
public class ConsumerService {
    ...
    public <T> void listen(T clazz) {
        ...
        log.info("ConsumerService.listen start: " + LocalDateTime.now()
                + " Thread: " + Thread.currentThread().getName() + " Topic: " + topic);
        try (KafkaConsumer<String, T> kafkaConsumer = new KafkaConsumer<>(properties)) {
            ...
            while (true) {
                ...
                for (ConsumerRecord<String, T> record : consumerRecords) {
                    ...
                    } else if (className.contains("TransactionWithFullName")) { // консьюмер на принимающей стороне обратного потока
                        TransactionWithFullName transactionWithFullName = (TransactionWithFullName) record.value();
                        log.info("Получено сообщение: {}", transactionWithFullName);
                    } else
